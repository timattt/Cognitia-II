...
void Circle::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,
    ...
    if (scalar <= (1.0 / 4))
        painter->setPen(QPen(QColorConstants::Svg::papayawhip, penSize));
    else if (scalar <= (1.0 / 2))
        painter->setPen(QPen(QColorConstants::Svg::moccasin, penSize));
    else if (scalar <= (3.0 / 4))
        painter->setPen(QPen(QColorConstants::Svg::lightsalmon, penSize));
    else
        painter->setPen(QPen(QColorConstants::Svg::orangered, penSize));
