...
class DBQuery:
	...
	def get_users(self, user_names, role=None):
		...
		self.cursor.execute('select * from users where name in ' + users_list + role_limit)
	...
	def get_students_folders_pathes(self, students):
		...
		self.cursor.execute('select * from student_x_progress_file where student in ' + students_list)
		return self.cursor.fetchall()

	def get_mentor_students(self, mentor):
		self.cursor.execute('select * from mentor_x_student where mentor = "' + mentor + '"')
		return self.cursor.fetchall()
